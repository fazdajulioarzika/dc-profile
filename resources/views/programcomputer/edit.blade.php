    ...
    <div class="min-h-screen">
        ...
        <form class="w-3/5 px-6 py-3 mt-8 ms-6 toright bg-white rounded-xl shadow-md" method="post"
            ...
            <div class="mb-5">
                ...
                block w-full p-2.5"
                    value="{{ old('name') ?? $computer->name }}" name="name" placeholder="Masukan nama program computer">
                @error('name')
            ...
            <div class="mb-5">
                ...
                <textarea id="description"
                    class="bg-gray-50 border @error('description') border-red-500 @enderror
                           text-gray-900 text-base rounded-lg focus:ring-blue-500 focus:border-blue-500
                           block w-full p-2.5"
                    name="description" placeholder="Masukan deskripsi program computer" rows="4">{{ old('description') ?? $computer->description }}</textarea>
                @error('description')
                    <div class="text-red-500 text-sm m-1">
                        {{ $message }}
                    </div>
            ...
            <div class="mb-5">
                ...
                <select name="icon" id="icon"
                    ...
                    <option value="webprogramming" {{ $computer->icon == 'webprogramming' ? 'selected' : '' }}> Coding
                    </option>
                    <option value="globe" {{ $computer->icon == 'globe' ? 'selected' : '' }}> Network</option>
                    <option value="pallet" {{ $computer->icon == 'pallet' ? 'selected' : '' }}> Design</option>
                    <option value="desktop" {{ $computer->icon == 'desktop' ? 'selected' : '' }}> Computer</option>
                    <option value="keyboard" {{ $computer->icon == 'keyboard' ? 'selected' : '' }}> Keyboard</option>
                </select>
                @error('icon')
                    <div class="text-red-500 text-sm m-1">
                        {{ $message }}
